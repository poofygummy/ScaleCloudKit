...
public enum SCKLogLevel: Int, CaseIterable, Identifiable, Comparable {
    // Logging is disabled.
    case disabled = 0
    ...
    // For Comparable
    public static func < (lhs: SCKLogLevel, rhs: SCKLogLevel) -> Bool {
        lhs.rawValue < rhs.rawValue
    }
}

/// Type for writes a emoji in writeLog(tag: ...)
public enum SCKLogTagEmoji: String {
    case error = "[ERROR]"
...
public final class SCKLogFileManager: @unchecked Sendable {

    // MARK: - Singleton

    /// Shared singleton instance of the log manager.
    public static let shared = SCKLogFileManager()

    /// Configures the shared logger instance.
    /// - Parameters:
    ///   - minLevel: The minimum log level to be recorded.
    public static func configure(logLevel: SCKLogLevel = .normal) {
    ...
    public var logLevel: SCKLogLevel
    ...
    private init(logLevel: SCKLogLevel = .normal, blacklist: [String]? = nil, whitelist: [String]? = nil) {
    ...
    /// - Parameters:
    ///   - logLevel: The SCKLogLevel { disabled .. verbose }
    private func setConfiguration(logLevel: SCKLogLevel) {
        self.logLevel = logLevel
    }
    ...
    public func writeLog(debug message: String, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
        writeLog("[DEBUG] \(message)", minimumLogLevel: minimumLogLevel, consoleOnly: consoleOnly)
    }

    public func writeLog(info message: String, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
        writeLog("[INFO] \(message)", minimumLogLevel: minimumLogLevel, consoleOnly: consoleOnly)
    }

    public func writeLog(warning message: String, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
        writeLog("[WARNING] \(message)", minimumLogLevel: minimumLogLevel, consoleOnly: consoleOnly)
    }

    public func writeLog(error message: String, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
        writeLog("[ERROR] \(message)", minimumLogLevel: minimumLogLevel, consoleOnly: consoleOnly)
    }

    public func writeLog(success message: String, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
        writeLog("[SUCCESS] \(message)", minimumLogLevel: minimumLogLevel, consoleOnly: consoleOnly)
    }

    public func writeLog(network message: String, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
        writeLog("[NETWORK] \(message)", minimumLogLevel: minimumLogLevel, consoleOnly: consoleOnly)
    }

    public func writeLog(start message: String, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
        writeLog("[START] \(message)", minimumLogLevel: minimumLogLevel, consoleOnly: consoleOnly)
    }

    public func writeLog(stop message: String, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
        writeLog("[STOP] \(message)", minimumLogLevel: minimumLogLevel, consoleOnly: consoleOnly)
    }

    public func writeLog(end message: String, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
        writeLog("[END] \(message)", minimumLogLevel: minimumLogLevel, consoleOnly: consoleOnly)
    }

    public func writeLog(cancel message: String, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
    ...
    public func writeLog(tag: String, emoji: SCKLogTagEmoji, message: String, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
    ...
    ///   - minimumLogLevel: set the minimun level for write the message
    ///   - consoleOnly: if true write the messa only in console
    public func writeLog(_ message: String?, emoji: SCKLogTagEmoji? = nil, minimumLogLevel: SCKLogLevel = .compact, consoleOnly: Bool = false) {
