...
public enum SCKTypeReachability: Int, Sendable {
    case unknown = 0
    case notReachable = 1
...
public protocol NextcloudKitDelegate: AnyObject, Sendable {
    ...
    func downloadComplete(fileName: String, serverUrl: String, etag: String?, date: Date?, dateLastModified: Date?, length: Int64, task: URLSessionTask, error: SCKError)
    func uploadComplete(fileName: String, serverUrl: String, ocId: String?, etag: String?, date: Date?, size: Int64, task: URLSessionTask, error: SCKError)
}
...
public struct SCKCommon: Sendable {
    public var nksessions = SynchronizedSCKSessionArray()
    public var delegate: NextcloudKitDelegate?
    public var groupIdentifier: String?
    public let typeIdentifiers: SCKTypeIdentifiers = .shared
    ...
    /// but returns the result and throws `SCKError` with the same domain/codes.
    ...
    public func chunkedFile(inputDirectory: String,
        ...
        if !fileManager.fileExists(atPath: outputDirectory, isDirectory: &isDirectory) {
            do {
                ...
                throw SCKError(errorCode: -2, errorDescription: "Failed to create the output directory for file chunks.")
            }
        }
        ...
        do {
            ...
            throw SCKError(errorCode: -1, errorDescription: "Failed to open the input file for reading.")
        }
        ...
        func checkCancelled() throws {
            if Task.isCancelled {
                throw SCKError(errorCode: -5, errorDescription: "Chunking was cancelled.")
            }
        }
        ...
        outerLoop: repeat {
            ...
            case 1:
                continue
            case -1:
                throw SCKError(errorCode: -1, errorDescription: "Failed to read data from the input file.")
            case -2:
                throw SCKError(errorCode: -2, errorDescription: "Failed to open the output chunk file for writing.")
            case -3:
                throw SCKError(errorCode: -3, errorDescription: "There is not enough available disk space to proceed.")
            case -4:
                throw SCKError(errorCode: -4, errorDescription: "Failed to write data to chunk file.")
            case -5:
                throw SCKError(errorCode: -5, errorDescription: "Chunking was cancelled.")
            default:
                continue
